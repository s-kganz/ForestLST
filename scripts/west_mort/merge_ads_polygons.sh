...
for f in $INPUT_DIRECTORY/*.gdb; do
    ...
    damage_sql="SELECT SHAPE,SURVEY_YEAR,DCA_CODE,DAMAGE_TYPE_CODE,PERCENT_MID,ifnull(PERCENT_MID, 10) AS SEVERITY FROM $damage_layer WHERE ST_Area(SHAPE) > 10000 AND (DAMAGE_TYPE_CODE = 2 OR DAMAGE_TYPE_CODE = 11) AND (DCA_CODE/1000 = 11 OR DCA_CODE/1000 = 15 OR DCA_CODE=50003)"
